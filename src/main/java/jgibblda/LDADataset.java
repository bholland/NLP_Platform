...
public class LDADataset {
	 ...
	 * @param idx index in the document array
	 */
	public void setDoc(String str, int idx, String documentLabel){
		if (0 <= idx && idx < M){
	...
	/**
	 * 
     * Read a dataset from the database using the passed function.
	 * @param connection: sql connection
	 * @param function: sql function to call. The function should take no parameters and return an int, string combination. 
	 * @return
	 * @throws SQLException 
	 */
	public static LDADataset readDataSet(Connection connection, String function) {
	    try {
    	    CallableStatement sp_call = connection.prepareCall(String.format("{call %s()}", function), ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ...
            while (rs.next()) {
                ...
                data.setDoc(text, index, String.format("%d", id));
                index++;
            }
            return data;
	...
	public static LDADataset readDataSet(Connection connection, String function, Dictionary dict) {
        try {
            CallableStatement sp_call = connection.prepareCall(String.format("{call %s()}", function), ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            sp_call.executeQuery();
