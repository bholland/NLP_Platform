...
public class Model {	
	...
	public boolean saveModelTheta(String filename){
		try{
			...
			for (int j = 0; j < K; j++){
                if (j+1 == K) {
                    writer.write(String.format("topic%3d", j));
                } else {
                    writer.write(String.format("topic%3d,", j));
                }
            }
            writer.write("\n");
			for (int i = 0; i < M; i++){
			    if (data.docs[i].label != null) {
			        writer.write(String.format("%s,", data.docs[i].label));
			    } else {
	...
	public boolean initNewModel(LDACmdOption option){
		...
		if (option.connection != null) {
		    data = LDADataset.readDataSet(option.connection, option.sql_function);
		} else {
	...
	public boolean initNewModel(LDACmdOption option, Model trnModel){
		...
		if (option.connection != null) {
		    dataset = LDADataset.readDataSet(option.connection, option.sql_function, trnModel.data.localDict);
		} else {
		    dataset = LDADataset.readDataSet(dir + File.separator + dfile, trnModel.data.localDict);
		}
		if (dataset == null){
